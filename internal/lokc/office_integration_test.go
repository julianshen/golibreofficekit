...
// LibreOffice installs SIGWINCH/SIGPIPE handlers without SA_ONSTACK.
// Go 1.14+ async preemption via SIGURG interacts with that and
// crashes the runtime. The Makefile's test-integration target sets
// GODEBUG=asyncpreemptoff=1 to disable async preemption; run this
// test via `make test-integration`.
